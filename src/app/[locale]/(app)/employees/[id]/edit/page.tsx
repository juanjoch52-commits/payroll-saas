...
export default async function EmployeeEditPage({
  ...
  const [{ data: employee }, { data: jurisdictions }, { data: subcontractors }, { data: billing }] =
    await Promise.all([
      supabase
        .from('employees')
        .select(
      ...
      // Bill rate: tabla privada (solo manager+ pasa el RLS).
      supabase.from('employee_billing').select('bill_rate_cents').eq('employee_id', id).maybeSingle(),
    ])

  if (!employee) notFound()
  const billRateCents = (billing as { bill_rate_cents: number | null } | null)?.bill_rate_cents ?? null
